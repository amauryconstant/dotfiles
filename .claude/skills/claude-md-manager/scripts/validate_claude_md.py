...
Validates CLAUDE.md files against Anthropic best practices and common anti-patterns.
"""
...
class CLAUDEValidator:
    ...
    def check_length(self):
        ...
        if line_count > 1000:
            self.issues.append((
                'ERROR',
                f'File is {line_count} lines. Strongly consider splitting into modular .claude/rules/ files (recommended max: 300 lines)',
                0
            ))
        elif line_count > 500:
            self.issues.append((
    ...
    def estimate_tokens(self):
        ...
        if estimated_tokens > 4000:
            self.issues.append((
                'ERROR',
                f'Estimated {estimated_tokens} tokens ({line_count} lines × 4). Critical: May cause Claude to ignore rules (recommended <1200 tokens)',
                0
            ))
    ...
    def check_anti_patterns(self):
        ...
        anti_patterns = [
            ...
            (r'(is a|are a|was a|were a).+(created by|developed by|written by)',
             'Historical background Claude doesn\'t need'),
        ...
        for line_num, line in enumerate(self.lines, 1):
            ...
            for pattern, message in anti_patterns:
                if re.search(pattern, lower_line):
                    self.issues.append((
                        'WARNING',
                        f'Possible anti-pattern: {message}',
                        line_num
                    ))
...
def find_claude_md_files(root_path: Path) -> List[Path]:
    ...
    return sorted(root_path.rglob('CLAUDE.md'))


def main():
    if len(sys.argv) < 2:
        print("Usage: python validate_claude_md.py <path>")
        print("  <path> can be a CLAUDE.md file or directory to search")
        sys.exit(1)
    ...
    print(f"Found {len(files_to_validate)} CLAUDE.md file(s) to validate\n")

    all_passed = True
    for filepath in files_to_validate:
        validator = CLAUDEValidator(filepath)
    ...
    print("\n" + "=" * 60)
    if all_passed:
        print("✅ All CLAUDE.md files passed validation")
        sys.exit(0)
    else:
        print("❌ Some CLAUDE.md files have errors")
        sys.exit(1)
