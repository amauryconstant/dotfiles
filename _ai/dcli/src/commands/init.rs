...
pub fn run(paths: &ConfigPaths, bootstrap_blackdon: bool, lua: bool, nix: bool, nix_init: bool) -> Result<()> {
    ...
    let pkg_manager_str = match pkg_manager_type {
        ...
        crate::config::PackageManagerType::Apt => "apt",
        crate::config::PackageManagerType::Dnf => "dnf",
    };
    ...
    let gitignore_content = if root_gitignore.exists() {
        let existing = fs::read_to_string(&root_gitignore)?;
        if !existing.contains("system-packages") {
            format!(
                "{}\n# System packages merged from host (auto-generated, host-specific)\nsystem-packages-*.yaml\n",
                existing
            )
        } else {
            existing
    ...
    if !root_gitignore.exists() || !fs::read_to_string(&root_gitignore)?.contains("system-packages")
    {
        println!("{} Updating .gitignore...", "→".blue());
        fs::write(&root_gitignore, gitignore_content)
            .context("Failed to create/update .gitignore")?;
        println!(
...
# Package manager: pacman (Arch), apt (Debian/Ubuntu), or dnf (Fedora/RHEL)
...
"#,
    ...
    // Create base.yaml in modules/ (new location) - distro-aware
    ...
    if !base_file.exists() {
        ...
        let base_content = match pkg_manager_type {
            crate::config::PackageManagerType::Pacman => {
                r#"# Base packages installed on all systems
...
"#
            ...
            crate::config::PackageManagerType::Apt => r#"# Base packages installed on all systems
# These packages are included regardless of host or modules
...
"#
            .to_string(),
            crate::config::PackageManagerType::Dnf => r#"# Base packages installed on all systems
# These packages are included regardless of host or modules

description: Base system packages

packages:
  # Essential build tools (uncomment if needed)
  # - "@Development Tools"
...
"#
    ...
    // Create host-specific FULL configuration file
    ...
    if !host_file.exists() {
        ...
        let pkg_manager_section = match pkg_manager_type {
            crate::config::PackageManagerType::Pacman => format!(
                r#"# AUR helper to use (auto-detects paru or yay if not specified)
...
#   devel: false                           # Set to true to always use --devel flag (updates -git packages)"#
            ...
            crate::config::PackageManagerType::Apt => format!(
                r#"# Update hooks (optional - run scripts before/after system updates)
# update_hooks:
#   pre_update: "scripts/pre-update.sh"   # Run before system update
#   post_update: "scripts/post-update.sh" # Run after update
#   behavior: ask                          # Options: ask, always, once, skip"#
            ),
            crate::config::PackageManagerType::Dnf => format!(
                r#"# Update hooks (optional - run scripts before/after system updates)
# update_hooks:
#   pre_update: "scripts/pre-update.sh"   # Run before system update
#   post_update: "scripts/post-update.sh" # Run after update
...
fn run_nix_config_init(paths: &ConfigPaths) -> Result<()> {
    println!(
        "{}",
        "=== Initializing dcli (Nix Mode) ===".blue()
    );
    println!();
    println!(
        "{}",
    ...
    if !crate::nix_eval::is_nix_installed() {
        ...
        println!(
            "{}",
            "  Or run: dcli init --nix-init".yellow()
        );
        println!();
...
fn run_nix_init(paths: &ConfigPaths) -> Result<()> {
    println!("{}", "=== Initializing Nix & Home Manager Integration ===".blue());
    println!();
    ...
    if crate::nix::is_nix_installed() {
        ...
        println!();
        println!("{}", "Note: You may need to log out and log back in for PATH changes to take effect.".yellow());
        println!("{}", "Then run 'dcli init --nix-init' again to continue.".yellow());
        return Ok(());
    }
    ...
    println!("  {} Using {} mode", "→".blue(), if use_flakes { "flake" } else { "channel" });
    ...
    if use_flakes {
        ...
        if !hm_dir.join("hosts").exists() {
            ...
            if old_dcli.exists() {
                println!("  {} Migrating from flat to per-host structure...", "→".blue());
            }
        }
        ...
        if !dcli_packages_path.exists() || is_new_host {
            crate::nix::generate_dcli_packages_nix(&[], &dcli_packages_path)?;
            println!("  {} Created hosts/{}/dcli-packages.nix", "✓".green(), hostname);
        }
    ...
    println!();
    println!("{}", "✓ Nix & Home Manager integration initialized!".green());
    println!();
    println!("Home Manager config location: {}", paths.home_manager_dir().display());
    if use_flakes {
        println!("Mode: Flakes (flake.nix + flake.lock)");
        println!();
        println!("Next steps:");
        println!("  1. Edit your config: {}", "nano ~/.config/dcli/home-manager/home.nix".cyan());
        println!("  2. Add nix packages to modules with {}", "type: nix".cyan());
        println!("  3. Run {} to apply changes", "dcli sync".cyan());
        println!("  4. Run {} to update flake inputs and apply", "dcli nix update".cyan());
    } else {
        ...
        println!("  2. Add nix packages to modules with {}", "type: nix".cyan());
        println!("  3. Run {} to apply changes", "dcli sync".cyan());
        println!("  4. Run {} to manually apply home-manager", "dcli nix switch".cyan());
    }
...
fn update_host_config_nix(paths: &ConfigPaths, use_flakes: bool) -> Result<()> {
    ...
    if config_path.extension().and_then(|e| e.to_str()) == Some("lua") {
        ...
        println!("  {} Lua config detected. Add this to your config:", "→".yellow());
        println!("    nix = {{");
    ...
    if config_path.extension().and_then(|e| e.to_str()) == Some("nix") {
        println!("  {} Nix config detected. Add this inside your host attribute set:", "→".yellow());
        println!("    nix = {{");
        println!("      enabled = true;");
        println!("      home_manager_enabled = true;");
        if use_flakes {
...
"#,
    ...
    std::fs::write(&config_path, updated)?;
    println!("  {} Added nix section to {}", "✓".green(), config_path.display());
...
fn ensure_nix_on_path() {
    ...
    let path_line: Option<String> = match shell_name {
        ...
        "bash" => {
            let _bashrc = dirs::home_dir()
                .map(|h| h.join(".bashrc"))
                .unwrap_or_else(|| std::path::PathBuf::from("~/.bashrc"));
            Some(format!(
                "\n# nix profile PATH set by dcli\nexport PATH=\"{}\"$PATH\n",
                format!("{}:", nix_profile_bin)
            ))
        }
        "zsh" => {
            let _zshrc = dirs::home_dir()
                .map(|h| h.join(".zshrc"))
    ...
    if let Some(line) = path_line {
        ...
        if let Ok(existing) = std::fs::read_to_string(&rc_file) {
            if !existing.contains("dcli") && !existing.contains(nix_profile_bin) {
                if std::fs::write(&rc_file, format!("{}{}", existing, line)).is_ok() {
                    println!(
                        "{} Added {} to {}",
                        "✓".green(),
                        nix_profile_bin.cyan(),
