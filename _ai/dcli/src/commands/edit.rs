...
fn discover_config_files(paths: &ConfigPaths) -> Result<Vec<PathBuf>> {
        ...
        })
    {
        ...
        // Only include YAML, Lua, or Nix files
        if path.is_file()
            && path
                .extension()
                .map(|s| s == "yaml" || s == "yml" || s == "lua" || s == "nix")
                .unwrap_or(false)
        {
    ...
    if config_files.is_empty() {
        anyhow::bail!(
            "No config files (YAML, Lua, or Nix) found in {}",
...
fn make_relative_display_path(path: &PathBuf, config_dir: &PathBuf) -> String {
