...
fn sync_single_dotfile(
    ...
    let already_backed_up = state
        .backed_up
        .iter()
        .any(|b| b.target == target.to_string_lossy().to_string() && b.module == *module_name);
    ...
    if target.is_symlink() || target.exists() {
        ...
        if target.is_symlink() {
            ...
            if !already_backed_up {
                ...
                // Already backed up before, just remove
                if target.is_dir() {
                    fs::remove_dir_all(target)
                        .context(format!("Failed to remove directory: {:?}", target))?;
                } else {
                    fs::remove_file(target)
...
fn print_sync_summary(stats: &SyncStats) {
    ...
    if total_processed > 0 {
        ...
        // Show actions taken (changes made)
        ...
        if stats.backups > 0 {
            println!(
                ...
                "✓".green(),
                stats.backups,
                if stats.backups == 1 { "" } else { "s" }
            );
            actions_shown = true;
        }

        // Show status information (dimmed to differentiate from actions)
        if stats.skipped > 0 {
            println!(
                "  {} {} dotfile{} already in sync",
                "✓".dimmed(),
                stats.skipped,
        ...
        // Show total summary if any actions were taken OR multiple items exist
        if actions_shown || total_processed > 1 {
            let total_symlinks = stats.created + stats.updated + stats.skipped;
            println!(
                "  {} {} total dotfile{} managed",
                "⊙".blue(),
                total_symlinks,
                if total_symlinks == 1 { "" } else { "s" }
            );
        }
    } else {
        // No dotfiles found in any enabled module
        println!("  {} No dotfiles configured", "ℹ".blue());
    }
}
...
fn expand_tilde(path: &str, home_dir: &str) -> Result<PathBuf> {
    if path.starts_with("~/") {
        Ok(PathBuf::from(home_dir).join(&path[2..]))
    } else if path == "~" {
