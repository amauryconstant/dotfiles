...
fn validate_module_content(table: &Table, path: &Path, result: &mut LuaValidationResult) {
    ...
    if let Ok(Value::Table(packages)) = table.get::<Value>("packages") {
        ...
        for pair in packages.pairs::<i64, Value>() {
                    ...
                    Value::Table(t) => {
                        ...
                        if t.get::<String>("name").is_err() {
                            result.add_error(LuaError {
                                ...
                                line: None,
                                hint: Some(
                                    "Use: { name = \"package-name\", type = \"pacman\" }"
                                        .to_string(),
                                ),
                            });
                        }
...
fn extract_module_config(path: &Path, table: &Table) -> Result<LuaModule> {
    ...
    // Extract deb packages (optional)
    let deb_packages: Vec<String> = table
        .get("deb_packages")
        .map_err(|e| anyhow!("Lua error: {}", e))
        .unwrap_or_default();
...
pub fn load_lua_directory_module(path: &Path) -> Result<LuaDirectoryModule> {
    ...
    let deb_packages: Vec<String> = result.get("deb_packages").unwrap_or_default();

    Ok(LuaDirectoryModule {
        manifest,
        packages,
        deb_packages,
    })
...
fn extract_config(table: &Table) -> Result<Config> {
    ...
    let nix: crate::config::NixConfig = if let Ok(nix_table) = table.get::<mlua::Table>("nix") {
        ...
        let nix_profile_enabled: bool = nix_table.get("nix_profile_enabled").unwrap_or(false);
        let flake_enabled: bool = nix_table.get("flake_enabled").unwrap_or(false);
        let nixpkgs_channel: String = nix_table.get("nixpkgs_channel").unwrap_or_else(|_| "nixpkgs-unstable".to_string());
        let home_manager_channel: String = nix_table.get("home_manager_channel").unwrap_or_else(|_| "release-25.05".to_string());
