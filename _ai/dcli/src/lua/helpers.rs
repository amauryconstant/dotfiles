...
fn create_util_helpers(lua: &Lua) -> Result<Table> {
    ...
    util.set(
        ...
        lua.create_function(|_, (target, source): (Table, Table)| {
            ...
            for pair in target.clone().pairs::<i64, Value>() {
                if let Ok((idx, _)) = pair {
                    max_idx = max_idx.max(idx);
                }
            }

            // Append items from source
            for pair in source.pairs::<i64, Value>() {
                if let Ok((_, v)) = pair {
                    max_idx += 1;
...
fn compare_versions(v1: &str, v2: &str) -> i32 {
    ...
    let parts1: Vec<&str> = v1.split(|c| c == '.' || c == '-' || c == '_').collect();
    let parts2: Vec<&str> = v2.split(|c| c == '.' || c == '-' || c == '_').collect();
