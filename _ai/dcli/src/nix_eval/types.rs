    ...
    if path.starts_with("/nix/store/") {
        ...
        if let Some(basename) = path.rsplit('/').next() {
            ...
            if basename.len() > 33 {
                ...
                // Try to strip a version suffix (last `-<digit>...` segment)
                if let Some(dash_pos) = rest.rfind('-') {
                    let after_dash = &rest[dash_pos + 1..];
                    // If the part after the last '-' starts with a digit, treat it as version
                    if after_dash.starts_with(|c: char| c.is_ascii_digit()) {
                        return rest[..dash_pos].to_string();
                    }
                }
...
impl NixConfigRaw {
    ...
    pub fn to_config(self) -> Config {
        let mut packages: Vec<PackageEntry> = self
            .packages
            .iter()
        ...
        let default_apps = if let Some(a) = self.default_apps {
            let mut apps = crate::config::DefaultAppsConfig::default();
            apps.scope = match a.scope.as_deref() {
                Some("user") => crate::config::DefaultsScope::User,
                _ => crate::config::DefaultsScope::System,
            };
            apps.browser = a.browser;
            apps.text_editor = a.text_editor;
            apps.file_manager = a.file_manager;
            apps.terminal = a.terminal;
            apps.video_player = a.video_player;
            apps.audio_player = a.audio_player;
            apps.image_viewer = a.image_viewer;
            apps.pdf_viewer = a.pdf_viewer;
            apps.mime_types = a.mime_types.unwrap_or_default();
            apps
        } else {
        ...
        let nix = if let Some(n) = self.nix {
            crate::config::NixConfig {
                ...
                nix_profile_enabled: n.nix_profile_enabled,
        ...
        let package_manager = self.package_manager.and_then(|s| match s.as_str() {
            ...
            "apt" => Some(crate::config::PackageManagerType::Apt),
            "dnf" => Some(crate::config::PackageManagerType::Dnf),
            _ => None,
        });
...
impl NixModuleRaw {
    pub fn to_nix_module(self, path: PathBuf) -> NixModule {
        ...
        NixModule {
            ...
            description: self.description,
            packages,
            deb_packages: self.deb_packages,
            services,
