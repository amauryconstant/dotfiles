...
pub fn load_nix_directory_module(
    path: &Path,
) -> Result<(ModuleManifest, Vec<PackageEntry>, Vec<String>)> {
    let facts = SystemFacts::collect();
    let value = evaluate_nix_file_to_json(path, &facts)
    ...
    let mut packages: Vec<PackageEntry> = raw.packages.iter().map(|e| e.to_package_entry()).collect();
    ...
    Ok((manifest, packages, raw.deb_packages))
}

#[allow(dead_code)]
pub fn validate_nix_module(path: &Path) -> Result<()> {
    let facts = SystemFacts::collect();
    let result = validate_nix_with_eval(path, &facts);
    if !result.valid {
...
{{
  ...
  # nix packages (installed via nix profile — standalone, no home-manager)
  nix_packages = [
    # "ripgrep"
    # "fd"
    # "bat"
