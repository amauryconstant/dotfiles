...
fn validate_config_structure(
    ...
    let valid_fields: &[&str] = &[
        "host", "description", "import", "enabled_modules", "packages",
        "flatpak_packages", "nix_packages", "exclude", "additional_packages", "auto_prune",
        "module_processing", "strict_package_order", "services", "enabled_service_profiles",
        "flatpak_scope", "update_hooks", "default_apps", "theming", "editor", "package_manager",
        "aur_helper", "sync_sudo", "auto_commit", "nix", "config_backups", "system_backups",
        "run_hooks_as_user",
    ];

    let module_valid_fields: &[&str] = &[
        "description", "packages", "flatpak_packages", "nix_packages", "deb_packages",
        "conflicts", "services", "pre_install_hook", "post_install_hook", "hook_behavior",
        "pre_hook_behavior", "post_hook_behavior", "post_disable_hook", "post_disable_behavior",
        "run_hooks_as_user", "dotfiles", "dotfiles_sync", "author", "version", "category", "tags",
...
fn generate_nix_error_hint(error: &str) -> Option<String> {
    ...
    if e.contains("syntax error") || e.contains("unexpected") {
        return Some(
            "Check for missing semicolons, brackets, or incorrect Nix syntax".to_string(),
        );
    }
...
pub fn parse_nix_config(value: serde_json::Value) -> Result<NixConfigRaw> {
    serde_json::from_value(value)
        .map_err(|e| anyhow!("Failed to parse nix config JSON: {}", e))
}

pub fn parse_nix_module(value: serde_json::Value) -> Result<NixModuleRaw> {
    serde_json::from_value(value)
        .map_err(|e| anyhow!("Failed to parse nix module JSON: {}", e))
}
