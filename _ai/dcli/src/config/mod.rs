...
pub struct Config {
    ...
    /// Package manager type: pacman (Arch), apt (Debian/Ubuntu), or dnf (Fedora/RHEL)
...
pub struct NixConfig {
    ...
    /// Use nix profile install for packages (standalone, no home-manager)
    #[serde(default)]
    pub nix_profile_enabled: bool,
...
impl Config {
    ...
    pub fn merge(&mut self, other: Config) {
        // Merge enabled_modules (deduplicate)
        ...
        // Merge packages (allow duplicates - package manager will handle)
        self.packages.extend(other.packages);

        // Merge additional_packages (backwards compat)
        self.additional_packages.extend(other.additional_packages);

        // Merge excludes (deduplicate)
        ...
        // Merge enabled_service_profiles (deduplicate)
        for profile in other.enabled_service_profiles {
            if !self.enabled_service_profiles.contains(&profile) {
                self.enabled_service_profiles.push(profile);
            }
        }
...
/// The type of a package entry (native system package, flatpak, deb, or nix)
#[derive(Debug, Clone, Serialize, Deserialize, PartialEq, Eq)]
pub enum PackageType {
    /// Native system package (pacman on Arch, apt on Debian, dnf on Fedora, etc.)
    #[serde(alias = "pacman", alias = "native", rename = "native")]
    Native,
    #[serde(rename = "flatpak")]
    Flatpak,
    /// Local .deb package file
    #[serde(rename = "deb")]
    Deb,
...
pub struct PackageList {
    ...
    /// List of local .deb packages to install (filename only, files go in debs/ subdirectory)
    #[serde(default)]
...
impl ModuleStructure {
    ...
    /// Get all deb package filenames from any format
    pub fn deb_packages(&self) -> Vec<String> {
        match self {
            ModuleStructure::Legacy { content, .. } => content.deb_packages.clone(),
            ModuleStructure::Directory(dir) => {
                let mut all_debs = Vec::new();
                for pkg_list in &dir.package_lists {
                    all_debs.extend(pkg_list.deb_packages.clone());
                }
                all_debs
            }
            ModuleStructure::Lua(lua) => lua.deb_packages.clone(),
            ModuleStructure::Nix(dyn_mod) => dyn_mod.deb_packages.clone(),
        }
    }
    ...
    /// Get sharing metadata for upload/download functionality
    /// Returns None for legacy modules (they must be converted to directory format)
    pub fn sharing_metadata(&self) -> Option<crate::sharing::SharingMetadata> {
        match self {
            ModuleStructure::Legacy { .. } => None,
            ModuleStructure::Directory(dir) => {
                let author = dir.manifest.author.clone()?;
                let version = dir.manifest.version.clone()?;
                Some(crate::sharing::SharingMetadata {
                    author,
                    version,
                    description: dir.manifest.description.clone(),
                    category: dir.manifest.category.clone(),
                    tags: dir.manifest.tags.clone(),
                    license: dir.manifest.license.clone(),
                    upstream_url: dir.manifest.upstream_url.clone(),
                })
            }
            ModuleStructure::Lua(lua) => {
                let author = lua.author.clone()?;
                let version = lua.version.clone()?;
                Some(crate::sharing::SharingMetadata {
                    author,
                    version,
                    description: lua.description.clone(),
                    category: lua.category.clone(),
                    tags: lua.tags.clone(),
                    license: lua.license.clone(),
                    upstream_url: lua.upstream_url.clone(),
                })
            }
            ModuleStructure::Nix(dyn_mod) => {
                let author = dyn_mod.author.clone()?;
                let version = dyn_mod.version.clone()?;
                Some(crate::sharing::SharingMetadata {
                    author,
                    version,
                    description: dyn_mod.description.clone(),
                    category: dyn_mod.category.clone(),
                    tags: dyn_mod.tags.clone(),
                    license: dyn_mod.license.clone(),
                    upstream_url: dyn_mod.upstream_url.clone(),
                })
            }
        }
    }

    /// Check if this module can be uploaded (has required sharing metadata)
    #[allow(dead_code)]
    pub fn can_upload(&self) -> bool {
        if let Some(meta) = self.sharing_metadata() {
            meta.validate().is_ok()
        } else {
            false
        }
...
pub struct ModuleManifest {
    ...
    // === Sharing metadata fields (for module upload/download) ===
...
/// Config mode: which advanced config format is active
...
/// Detect the active config mode by checking which config files exist.
/// Errors if both config.lua and config.nix exist.
#[allow(dead_code)]
pub fn detect_config_mode(paths: &ConfigPaths) -> Result<ConfigMode> {
    let lua_config = paths.config_dir.join("config.lua");
    let nix_config = paths.config_dir.join("config.nix");
    let yaml_config = &paths.config_file;

    let has_lua = lua_config.exists();
    let has_nix = nix_config.exists();
    let has_yaml = yaml_config.exists();
...
pub fn resolve_config_path(paths: &ConfigPaths) -> Result<PathBuf> {
    ...
    if nix_config_file.exists() {
        if crate::nix_eval::is_nix_installed() {
            if let Some(hostname) = crate::nix_eval::detect_pointer_nix_config_file(&nix_config_file)? {
                let host_file = paths.host_packages_file(&hostname);
                if !host_file.exists() {
                    anyhow::bail!(
...
pub fn declared_packages_paths(paths: &ConfigPaths) -> Result<(PathBuf, PathBuf)> {
    ...
    let config_path = resolve_config_path(paths)?;
    let host_filename = config_path.file_name().and_then(|n| n.to_str()).unwrap_or("");
...
fn load_config_from_file(paths: &ConfigPaths, file_path: &Path) -> Result<Config> {
    ...
    let config = match extension {
        Some("nix") => {
            crate::nix_eval::load_nix_config(file_path)?
        }
        Some("lua") => {
...
pub fn load_module<P: AsRef<Path>>(path: P) -> Result<ModuleStructure> {
    ...
    if path.is_file() {
        ...
        let mut inline_deb_packages: Vec<String> = Vec::new();
        let manifest: ModuleManifest = if nix_manifest_path.exists() {
            log::debug!("Loading Nix manifest: {:?}", nix_manifest_path);
            let (manifest, pkgs, debs) =
                crate::nix_eval::load_nix_directory_module(&nix_manifest_path)?;
            inline_packages = pkgs;
            inline_deb_packages = debs;
            manifest
            ...
            inline_deb_packages = lua_dir.deb_packages;
            lua_dir.manifest
        ...
        if !inline_packages.is_empty() || !inline_deb_packages.is_empty() {
...
pub fn validate_module(module: &ModuleStructure, module_name: &str) -> ModuleValidationResult {
    ...
    match module {
        ModuleStructure::Legacy { content, .. } => {
            ...
            // Validate deb packages end with .deb extension
            for deb in &content.deb_packages {
                if !deb.ends_with(".deb") {
                    result.errors.push(format!(
                        "deb package '{}' must end with .deb extension",
                        deb
                    ));
                }
            }
        ...
        ModuleStructure::Directory(dir) => {
            ...
            // Validate deb packages: check extension and existence
            let debs_dir = dir.root.join("debs");
            for pkg_list in &dir.package_lists {
                for deb in &pkg_list.deb_packages {
                    if !deb.ends_with(".deb") {
                        result.errors.push(format!(
                            "deb package '{}' must end with .deb extension",
                            deb
                    ...
                    // Warn if debs/ directory doesn't exist but deb_packages are specified
                    if !debs_dir.exists() && !pkg_list.deb_packages.is_empty() {
                        result.warnings.push(format!(
                            "debs/ directory not found but deb_packages are specified"
                        ));
                        break;
                    }
                    ...
                    if debs_dir.exists() {
                        let deb_path = debs_dir.join(deb);
                        if !deb_path.exists() {
                            result
                                .errors
                                .push(format!("deb package file not found: debs/{}", deb));
                        }
                    }
                }
            }
        ...
        ModuleStructure::Lua(lua) => {
            ...
            // Validate deb packages: check extension and existence
            let lua_parent = lua.path.parent();
            let debs_dir = lua_parent.map(|p| p.join("debs"));
            for deb in &lua.deb_packages {
                if !deb.ends_with(".deb") {
                    result.errors.push(format!(
                        "deb package '{}' must end with .deb extension",
                        deb
                ...
                // Check if deb file exists
                if let Some(ref debs) = debs_dir {
                    if debs.exists() {
                        let deb_path = debs.join(deb);
                        if !deb_path.exists() {
                            result
                                .errors
                                .push(format!("deb package file not found: debs/{}", deb));
...
pub fn detect_package_manager_type() -> Result<PackageManagerType> {
    ...
    if let Ok(content) = std::fs::read_to_string("/etc/os-release") {
        ...
        if let Some(ref id) = id {
            ...
            match id.as_str() {
                ...
                "debian" | "ubuntu" | "pop" | "linuxmint" | "elementary" | "zorin" | "pikaos"
                | "pika" | "neon" | "kali" | "raspbian" | "mx" => {
    ...
    // Fallback: check which package manager binaries exist
    ...
    anyhow::bail!(
        "Could not detect package manager. Please set 'package_manager: pacman', 'package_manager: apt', or 'package_manager: dnf' in your config.yaml."
    )
}
