...
pub enum Screen {
    ...
    Upload(UploadScreenState),
    Download(DownloadScreenState),
}
...
impl Screen {
    ...
    pub fn handle_key(&mut self, key: KeyEvent) -> Result<Option<ScreenAction>> {
        match self {
            ...
            Screen::Upload(s) => s.handle_key(key),
            Screen::Download(s) => s.handle_key(key),
        }
    }
    ...
    pub fn render(
        ...
        match self {
            ...
            Screen::Upload(s) => s.render(paths, config, frame, area),
            Screen::Download(s) => s.render(paths, config, frame, area),
        }
    }
    ...
    pub fn on_activate(&mut self, paths: &ConfigPaths, config: &Config) -> Result<()> {
        match self {
            ...
            Screen::Upload(s) => s.on_activate(paths, config),
            Screen::Download(s) => s.on_activate(paths, config),
        }
    }
}

// Re-export screen states
pub use download::DownloadScreenState;
...
pub use upload::UploadScreenState;
