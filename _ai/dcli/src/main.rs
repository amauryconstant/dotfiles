...
enum ModuleAction {
    ...
    /// Upload module(s) to the community repository
    Upload {
        /// Module name (if not provided, shows interactive TUI)
        name: Option<String>,

        /// Set up Git credentials for module uploads
        #[arg(long)]
        git: bool,
    },

    /// Download module(s) from the community repository
    Download {
        /// Module name (if not provided, shows interactive TUI)
        name: Option<String>,
...
fn main() -> Result<()> {
    ...
    match cli.command {
        Commands::Init { bd, adv, nix, nix_init } => {
        ...
        Commands::Module { action } => match action {
            ...
            ModuleAction::Upload { name, git } => {
                if git {
                    commands::sharing::setup_git(cli.json)?;
                } else if let Some(name) = name {
                    commands::sharing::upload(&paths, &name, cli.json)?;
                } else {
                    // Launch TUI upload screen
                    tui::run_upload(paths)?;
                }
            }
            ModuleAction::Download { name, category } => {
                if let Some(name) = name {
                    commands::sharing::download(&paths, &name, category.as_deref(), cli.json)?;
                } else {
                    // Launch TUI download screen
                    tui::run_download(paths)?;
                }
            }
        ...
        Commands::Source { action } => {
            // Source builds require pacman (makepkg/PKGBUILD)
            if let Ok(config) = config::load_config(&paths) {
                let pm = config::resolve_package_manager(&config).ok();
                if pm == Some(config::PackageManagerType::Apt) || pm == Some(config::PackageManagerType::Dnf) {
                    eprintln!(
                        "{}",
                        "Error: Source builds (makepkg/PKGBUILD) are only supported on Arch-based systems."
                    );
                    eprintln!(
                        "{}",
                        "This feature is not available with the current package manager."
                    );
                    std::process::exit(1);
                }
            }
