...
pub trait PkgBackend {
    ...
    /// List all available packages (for search/fzf)
    ...
    /// Human-readable name for this backend (for log messages)
    fn name(&self) -> &str;

    /// Install local .deb packages (only supported on apt-based systems)
    /// Default implementation returns an error for non-apt backends
    fn install_deb_packages(&self, _deb_paths: &[&str]) -> Result<bool> {
        anyhow::bail!("Installing .deb packages is only supported on apt-based systems")
    }
}
...
pub fn create_backend(config: &Config) -> Result<Box<dyn PkgBackend>> {
    ...
    match pm_type {
        ...
        PackageManagerType::Apt => Ok(Box::new(apt::AptBackend::new())),
        PackageManagerType::Dnf => Ok(Box::new(dnf::DnfBackend::new())),
    }
}
