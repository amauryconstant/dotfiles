...
impl ModuleManager {
    ...
    pub fn list_modules(&self) -> Result<Vec<ModuleInfo>> {
            ...
            .filter(|e| e.file_type().is_dir())
        {
            ...
            // Check for module.lua, module.yaml, and module.nix
            if path.join("module.lua").exists()
        ...
        // Find all legacy YAML, Lua, and Nix files
        for entry in WalkDir::new(&modules_dir)
            .max_depth(3)
            .follow_links(false)
            .into_iter()
            ...
            })
        {
            ...
            // Skip module.yaml, module.lua, and module.nix files (they're part of directory modules)
            if path.file_name() == Some(std::ffi::OsStr::new("module.yaml"))
                || path.file_name() == Some(std::ffi::OsStr::new("module.lua"))
                || path.file_name() == Some(std::ffi::OsStr::new("module.nix"))
            {
                continue;
            }
    ...
    pub fn resolve_module_path(&self, module_input: &str) -> Result<String> {
        ...
        // Check for naming conflicts (yaml + lua + nix + directory with same base name)
        ...
        let nix_path = modules_dir.join(format!("{}.nix", base_name));
        let dir_path = modules_dir.join(base_name);

        let yaml_exists = yaml_path.exists();
        let lua_exists = lua_path.exists();
        let nix_exists = nix_path.exists();
        let dir_exists = dir_path.exists()
            && (dir_path.join("module.lua").exists()
                || dir_path.join("module.yaml").exists()
                || dir_path.join("module.nix").exists());

        let count = [yaml_exists, lua_exists, nix_exists, dir_exists]
            .iter()
            .filter(|&&x| x)
            .count();
        ...
        if module_input.contains('/') {
            ...
            let module_nix = modules_dir.join(format!("{}.nix", module_input));
            ...
            // Prefer directory module over file, then yaml, then lua, then nix
        ...
        // Search for file modules (yaml, lua, and nix)
        for entry in WalkDir::new(&modules_dir)
            .max_depth(3)
            .follow_links(false)
            .into_iter()
            ...
            .filter(|e| e.file_type().is_dir())
        {
            ...
            // Check if it's a directory module (has module.lua, module.yaml, or module.nix)
            if !path.join("module.lua").exists()
                && !path.join("module.yaml").exists()
                && !path.join("module.nix").exists()
            {
    ...
    pub fn check_conflicts(
        ...
        // Resolve module path for any format (yaml, lua, nix, or directory)
        let modules_dir = self.paths.modules_dir();
