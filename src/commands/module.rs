    ...
    if json {
        ...
        for category in categories {
            ...
            for (module, is_enabled) in &categorized[category] {
                let short_name = module.name.split('/').last().unwrap();
                print!("  ");
...
pub fn disable(paths: &ConfigPaths, module_name: &str, json: bool) -> Result<()> {
    ...
    if !json {
        ...
        let module_nix = modules_dir.join(format!("{}.nix", resolved_name));
...
fn save_config(paths: &ConfigPaths, config: &crate::config::Config) -> Result<()> {
    ...
    // If config.yaml/config.lua/config.nix is a pointer (minimal), save to host file
    // Otherwise, save to config file (legacy mode)
    let save_path = if let Ok(pointer_content) = std::fs::read_to_string(&paths.config_file) {
        if let Ok(pointer_config) = serde_yaml::from_str::<crate::config::Config>(&pointer_content)
        {
            ...
            // Can't parse as YAML - check if it's a Lua or Nix pointer config
            let ext = paths.config_file.extension().and_then(|e| e.to_str());
            if ext == Some("lua") || ext == Some("nix") {
                paths.host_packages_file(&config.host)
            } else {
                paths.config_file.clone()
            }
    ...
    // Check if target is a Lua or Nix file - we cannot auto-modify these configs
    let ext = save_path.extension().and_then(|e| e.to_str());
    if ext == Some("lua") || ext == Some("nix") {
        let config_type = if ext == Some("lua") { "Lua" } else { "Nix" };
        println!(
            "{}",
            format!(
                "⚠️  Warning: Cannot automatically save changes to {} file: {}\n\
                 {} configs contain code and cannot be auto-modified.\n\
...
pub fn create(paths: &ConfigPaths, module_path: &str, force_lua: bool, force_nix: bool) -> Result<()> {
    ...
    let modules_dir = paths.modules_dir();
    let module_file_path = if module_path.ends_with(".lua") || module_path.ends_with(".yaml") || module_path.ends_with(".nix") {
