...
fn run_packages_merge(paths: &ConfigPaths, dry_run: bool, include_deps: bool) -> Result<()> {
    ...
    let (preferred_declared, fallback_declared) =
        crate::config::declared_packages_paths(paths)?;
    ...
    let mut system_list = if packages_file.exists() {
        crate::config::load_package_list_any(&packages_file)?
    } else {
...
fn run_services_merge(paths: &ConfigPaths, dry_run: bool, user_scope: bool) -> Result<()> {
    ...
    let mut host_config = if host_file.exists() {
        match host_file.extension().and_then(|e| e.to_str()) {
            Some("lua") => {
                crate::lua::load_lua_config(&host_file).context("Failed to parse host configuration")?
            }
            Some("nix") => {
                crate::nix_eval::load_nix_config(&host_file).context("Failed to parse host configuration")?
            }
            _ => {
    ...
    match host_file.extension().and_then(|e| e.to_str()) {
        ...
        Some("nix") => {
            anyhow::bail!(
                "Cannot automatically save service changes to Nix file: {}\n\n\
                 Nix configs contain code and cannot be auto-modified.\n\
                 Please edit the file manually to add these services:\n  - {}",
                host_file.display(),
                unmanaged.join("\n  - ")
            );
        }
...
fn run_defaults_merge(paths: &ConfigPaths, dry_run: bool) -> Result<()> {
    ...
    for (category, desktop_file) in &unmanaged {
        ...
        match category.as_str() {
            "browser" => {
                if host_config.default_apps.browser.is_none() {
                    host_config.default_apps.browser = Some(clean_name);
                    added_count += 1;
                }
            }
            "text_editor" => {
                if host_config.default_apps.text_editor.is_none() {
                    host_config.default_apps.text_editor = Some(clean_name);
                    added_count += 1;
                }
            }
            "file_manager" => {
                if host_config.default_apps.file_manager.is_none() {
                    host_config.default_apps.file_manager = Some(clean_name);
                    added_count += 1;
                }
            }
            "terminal" => {
                if host_config.default_apps.terminal.is_none() {
                    host_config.default_apps.terminal = Some(clean_name);
                    added_count += 1;
                }
            }
            "video_player" => {
                if host_config.default_apps.video_player.is_none() {
                    host_config.default_apps.video_player = Some(clean_name);
                    added_count += 1;
                }
            }
            "audio_player" => {
                if host_config.default_apps.audio_player.is_none() {
                    host_config.default_apps.audio_player = Some(clean_name);
                    added_count += 1;
                }
            }
            "image_viewer" => {
                if host_config.default_apps.image_viewer.is_none() {
                    host_config.default_apps.image_viewer = Some(clean_name);
                    added_count += 1;
                }
            }
            "pdf_viewer" => {
                if host_config.default_apps.pdf_viewer.is_none() {
    ...
    match host_file.extension().and_then(|e| e.to_str()) {
        ...
        Some("nix") => {
            anyhow::bail!(
                "Cannot automatically save default app changes to Nix file: {}\n\n\
                 Nix configs contain code and cannot be auto-modified.\n\
                 Please edit the file manually to add these defaults:\n  - {}",
                host_file.display(),
                unmanaged.iter().map(|(k, v)| format!("{}: {}", k, v)).collect::<Vec<_>>().join("\n  - ")
            );
        }
        _ => {
            // Save as YAML (original behavior)
