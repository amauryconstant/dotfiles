...
pub fn run(paths: &ConfigPaths, dry_run: bool) -> Result<()> {
    ...
    if old_modules.exists() {
        actions.push(format!("Move packages/modules/ → modules/"));
    }
    if old_hosts.exists() {
        actions.push(format!("Move packages/hosts/ → hosts/"));
    }
    if old_base.exists() {
        actions.push(format!("Move packages/base.yaml → modules/base.yaml"));
    }
