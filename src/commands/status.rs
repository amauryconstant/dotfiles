...
pub fn run(paths: &ConfigPaths, json: bool) -> Result<()> {
    ...
    let mut installed_flatpaks: Vec<String> = pkg_manager.get_installed_flatpaks("--user").unwrap_or_default();
    let system_flatpaks = pkg_manager.get_installed_flatpaks("--system").unwrap_or_default();
    ...
    let pm_label = match pm_type {
        ...
        PackageManagerType::Apt => "dpkg/apt packages",
        PackageManagerType::Dnf => "rpm/dnf packages",
    };
    ...
    if json {
        ...
        if nix_installed_count > 0 {
            println!("  Nix packages: {}", nix_installed_count.to_string().green());
        }
        ...
        if config.nix.enabled || config.nix.home_manager_enabled {
            ...
            println!("  Nix enabled: {}", if config.nix.enabled { "yes".green().to_string() } else { "no".yellow().to_string() });
            println!("  Home Manager enabled: {}", if config.nix.home_manager_enabled { "yes".green().to_string() } else { "no".yellow().to_string() });
            println!("  Flake enabled: {}", if config.nix.flake_enabled { "yes".green().to_string() } else { "no".to_string() });
            println!("  Nixpkgs channel/input: {}", config.nix.nixpkgs_channel);
            println!("  Home Manager channel/input: {}", config.nix.home_manager_channel);
            ...
            if config.nix.flake_enabled {
                ...
                println!("  flake.nix: {}", if hn.join("flake.nix").exists() { "exists".green().to_string() } else { "not found".yellow().to_string() });
                println!("  flake.lock: {}", if hn.join("flake.lock").exists() { "exists".green().to_string() } else { "not found".yellow().to_string() });
            }
        }
    }
