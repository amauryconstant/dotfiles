...
/// Install nix (and optionally home-manager)
pub fn install(paths: &ConfigPaths) -> Result<()> {
    println!("{}", "=== Installing Nix ===".blue().bold());
    println!();
    ...
    // Only install home-manager if explicitly enabled in config
    if config.nix.home_manager_enabled {
        ...
        println!("{} Home Manager not enabled in config, skipping installation", "→".blue());
        println!("  Set nix.home_manager_enabled = true to enable it.");
    }

    println!();
    println!("{}", "✓ Nix setup complete!".green());
    println!();

    if config.nix.home_manager_enabled {
        println!("Next steps:");
        println!("  1. Run {} to set up dcli home-manager config", "dcli init --nix-init".cyan());
        println!("  2. Or manually create {}", "~/.config/dcli/home-manager/home.nix".cyan());
    } else if config.nix.nix_profile_enabled {
        println!("Next steps:");
        println!("  1. Add nix_packages to your host or module config (e.g., nix_packages = [\"ripgrep\"])");
        println!("  2. Run {} to install packages via nix profile", "dcli sync".cyan());
    }
...
pub fn status(paths: &ConfigPaths, json: bool) -> Result<()> {
    ...
    if nix_status.nix_installed {
        println!("  {} Installed: {}", "✓".green(), nix_status.nix_version.as_deref().unwrap_or("unknown"));
        println!("  {} Daemon: {}", "✓".green(), if nix_status.daemon_running { "running".green().to_string() } else { "not running".yellow().to_string() });
    } else {
    ...
    if nix_status.hm_installed {
        println!("  {} Installed: {}", "✓".green(), nix_status.hm_version.as_deref().unwrap_or("unknown"));
        println!("  {} Enabled in config: {}", "✓".green(), if config.nix.home_manager_enabled { "yes".green().to_string() } else { "no".yellow().to_string() });
        println!("  {} Flake enabled: {}", "✓".green(), if nix_status.flake_enabled { "yes".green().to_string() } else { "no".to_string() });
        if nix_status.flake_enabled {
            println!("  {} flake.nix: {}", "✓".green(), if nix_status.flake_nix_exists { "exists".green().to_string() } else { "not found".yellow().to_string() });
        }
        println!("  {} home.nix: {}", "✓".green(), if nix_status.home_nix_exists { "exists".green().to_string() } else { "not found".yellow().to_string() });
        println!("  {} dcli-packages.nix: {}", "✓".green(), if nix_status.dcli_packages_exists { "exists".green().to_string() } else { "not found".yellow().to_string() });
    } else {
        println!("  {} Not installed", "✗".red());
    }
