...
fn run_internal(paths: &ConfigPaths, check_packages: bool, json: bool, quiet: bool) -> Result<()> {
    if !json && !quiet {
        println!(
            "{}",
            "=== Validating dcli Config ===".blue().bold()
        );
        println!();
    }
    ...
    if !has_config {
        ...
        if json {
            ...
            println!("  {} config file not found (config.yaml, config.lua, or config.nix)", "✗".red());
        }
    } else {
    ...
    if !paths.packages_dir.exists() {
        ...
        // Check for base packages file (base.yaml, base.lua, or base.nix)
        let base_file = paths.base_packages_file();
        if base_file.exists() {
            let base_name = base_file.file_name().unwrap().to_string_lossy();
            match std::fs::read_to_string(&base_file) {
                Ok(content) => {
                    let is_valid = match base_file.extension().and_then(|e| e.to_str()) {
                        Some("lua") => crate::lua::validate_lua_module_detailed(&base_file).valid,
                        Some("nix") => crate::nix_eval::validate_nix_module_detailed(&base_file).valid,
                        _ => serde_yaml::from_str::<serde_yaml::Value>(&content).is_ok(),
                    };
                    if is_valid {
                        if !json && !quiet {
                            println!("  {} {} is valid", "✓".green(), base_name);
                        }
                        checks.push(json!({
                            "check": "base_packages",
                            "status": "ok"
                        ...
                        if json {
                            checks.push(json!({
                                "check": "base_packages",
                                "status": "error",
                                "message": format!("{} has invalid syntax", base_name)
                            }));
                        } else {
                            println!("  {} {} has invalid syntax", "✗".red(), base_name);
                        }
                    }
                }
                Err(e) => {
                    ...
                    if json {
                        checks.push(json!({
                            "check": "base_packages",
                            "status": "error",
                            "message": format!("Failed to read {}: {}", base_name, e)
                        }));
                    } else {
                        println!("  {} Failed to read {}: {}", "✗".red(), base_name, e);
                    }
                }
            }
        } else {
            ...
            if json {
                checks.push(json!({
                    "check": "base_packages",
                    "status": "warning",
                    "message": "base packages file not found (optional)"
                }));
            } else {
                println!("  {} base packages file not found (optional)", "⚠".yellow());
            }
        }
    ...
    if !modules_dir.exists() {
            ...
            .filter_map(|e| e.ok())
        {
            ...
            // (i.e., if any ancestor directory contains a module manifest)
            ...
            while let Some(dir) = current {
                ...
                if dir.join("module.yaml").exists()
                    || dir.join("module.lua").exists()
                    || dir.join("module.nix").exists()
                {
