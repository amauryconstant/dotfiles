...
pub fn run(paths: &ConfigPaths, package_name: &str) -> Result<()> {
    ...
    if system_packages_file.exists() {
        if let Ok(mut pkg_list) = load_package_list_any(&system_packages_file) {
            if let Some(pos) = pkg_list
                .packages
                .iter()
                .position(|p| p.name() == package_name)
            {
                pkg_list.packages.remove(pos);
                write_package_list_any(&system_packages_file, &pkg_list)
    ...
    for module_name in &config.enabled_modules {
                ...
                || module_dir.join("module.nix").exists())
        {
            ...
            } else if module_dir.join("module.nix").exists() {
                module_dir.join("module.nix")
            } else {
