...
pub fn run(paths: &ConfigPaths, no_backup: bool, no_hooks: bool, devel: bool) -> Result<()> {
    ...
    // Update Nix/Home Manager if enabled
    if config.nix.enabled && config.nix.home_manager_enabled {
        if nix::is_nix_installed() && nix::is_home_manager_installed() {
            println!();
            println!("{}", "=== Nix / Home Manager Update ===".blue().bold());

            match nix::home_manager_update(paths, &config) {
                Ok(()) => {
                    println!();
                    println!("{}", "✓ Nix / Home Manager update complete!".green());
                }
                Err(e) => {
                    println!();
                    println!(
                        "{}",
                        format!("⚠ Nix / Home Manager update completed with errors: {}", e).yellow()
                    );
                    println!("{}", "  Continuing with system update...".yellow());
                }
            }
        } else {
            println!();
            println!(
                "{}",
                "⚠ Nix/Home Manager is enabled in config but not installed. Run 'dcli nix install' first."
                    .yellow()
            );
        }
    }

    // Update Nix Profile if enabled (standalone, no home-manager)
    if config.nix.enabled && config.nix.nix_profile_enabled && !config.nix.home_manager_enabled {
        if nix::is_nix_installed() {
            println!();
            println!("{}", "=== Nix Profile Update ===".blue().bold());

            // If flakes are enabled, update flake inputs first
            if config.nix.flake_enabled {
                println!("{}", "  Updating nixpkgs flake input...".blue());
                let flake_status = std::process::Command::new("nix")
                    .args(["flake", "update", "nixpkgs"])
                    .stdin(std::process::Stdio::inherit())
                    .stdout(std::process::Stdio::inherit())
                    .stderr(std::process::Stdio::inherit())
                    .status()
                    .context("Failed to update nixpkgs flake input")?;

                if !flake_status.success() {
                    println!("{}", "⚠ Failed to update nixpkgs flake input, continuing with upgrade...".yellow());
                }
                println!();
            }

            match nix::nix_profile_upgrade(false) {
                Ok(true) => {
                    println!();
                    println!("{}", "✓ Nix Profile update complete!".green());
                }
                Ok(false) => {
                    println!();
                    println!(
                        "{}",
                        "⚠ Nix Profile update completed with errors".yellow()
                    );
                }
                Err(e) => {
                    println!();
                    println!(
                        "{}",
                        format!("⚠ Nix Profile update failed: {}", e).yellow()
                    );
                }
            }
        } else {
            ...
            println!(
                "{}",
                "⚠ Nix is enabled in config but not installed. Install Nix to use nix profile."
                    .yellow()
            );
        }
    }
