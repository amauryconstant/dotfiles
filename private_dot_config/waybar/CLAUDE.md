...
| **Voxtype** | `@fg-muted` (idle) | `@accent-error` (recording), `@accent-info` (streaming), `@accent-warning` (transcribing), `@accent-error` dimmed (stopped) | Mic activity + daemon lifecycle (`voxtype status --follow`) |
