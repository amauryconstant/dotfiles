...
if command -v wofi >/dev/null 2>&1; then
    ...
    case "$TEMP" in
        "6500K (Daylight)")
            ...
            notify-send "󰛨 Nightlight" "Set to Daylight (6500K)"
            ;;
        "6000K (Cool)")
            hyprctl keyword decoration:col.shadow 0xee221111
            notify-send "󰛨 Nightlight" "Set to Cool (6000K)"
            ;;
        "5500K (Neutral)")
            hyprctl keyword decoration:col.shadow 0xee332211
            notify-send "󰛨 Nightlight" "Set to Neutral (5500K)"
            ;;
        "5000K (Warm)")
            hyprctl keyword decoration:col.shadow 0xee443311
            notify-send "󰛨 Nightlight" "Set to Warm (5000K)"
            ;;
        "4500K (Warmer)")
            hyprctl keyword decoration:col.shadow 0xee554411
            notify-send "󰛨 Nightlight" "Set to Warmer (4500K)"
            ;;
        "4000K (Evening)")
            hyprctl keyword decoration:col.shadow 0xee665511
            notify-send "󰛨 Nightlight" "Set to Evening (4000K)"
            ;;
        "3500K (Night)")
            hyprctl keyword decoration:col.shadow 0xee776611
            notify-send "󰛨 Nightlight" "Set to Night (3500K)"
            ;;
    esac
else
    notify-send "󰛨 Nightlight" "wofi not available for configuration"
fi